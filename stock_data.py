...
class StockData:
    """
    Class representing stock data for a specific ticker symbol.
    Handles interactions with the yfinance API.
    """
    ...
    def get_history(self, period: str = "1mo") -> pd.DataFrame:
        ...
        """
        logger.debug(f"Fetching data for {self.ticker} with period {period}")
        try:
    ...
    def _generate_test_data(self, period: str = "1mo") -> pd.DataFrame:
        """
        Generates test data when the API fails.
        This is a fallback for development and testing only.
        
        Args:
            period: Time period to generate data for
            
        Returns:
            A pandas DataFrame with sample stock price data
        """
        import numpy as np
    ...
    def get_company_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error getting company info for {self.ticker}: {str(e)}")
            return {}
            
    def _translate_field(self, text: str) -> str:
        """
        Translate text to Czech language
